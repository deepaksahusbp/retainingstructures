...
//Message:
    ...
    $mail->Subject = 'Design Results - Counterfort Retaining Wall';

    //$mail->Body = '<h3>hii</h3>';
    $mail->Body = '<h1>Counterfort Retaining Wall</h1>';
    ...
    $mail->AddEmbeddedImage('images/MOD1.png', 'cp');
    $mail->Body.='<center><img src="cid:cp" /></center><br><br>';
    $mail->Body.= '<table style="border: 1px solid black;"> <tr>
    ...
    //     $totalPrice+=$row["Price"]*$row["quantity"];
         $mail->Body.='<tr> <td>'.$_SESSION['DOF'].'</td> <td>'.$_SESSION['LOB'].'</td> <td>'.$_SESSION['TOP'].'</td> <td>'.$_SESSION['SW'].'</td> <td>'.$_SESSION['LOH'].'</td> <td>'.$_SESSION['Q'].'</td> <td>'.$_SESSION['LFF2'].'</td> <td>'.$_SESSION['CD'].'</td> <td>'.$_SESSION['FP2'].'</td> </tr> <br>';
    ...
    $mail->AddEmbeddedImage('images/3.jpg', 'cld');
    $mail->Body.='<center><img src="cid:cld" /></center><br><br>';
    $mail->Body.= '<table style="border: 1px solid black;"> <tr>
        <th>UDL</th>
    ...
    // while ($row = mysqli_fetch_array($resultOrder)){
    //     $totalPrice+=$row["Price"]*$row["quantity"];
         $mail->Body.='<tr> <td>'.$_SESSION['UDL'].'</td> <td>'.$_SESSION['PMAX'].'</td> <td>'.$_SESSION['PMIN'].'</td> <td>'.$_SESSION['SBC'].'</td> <td>'.$_SESSION['PB'].'</td> <td>'.$_SESSION['PC'].'</td> <td>'.$_SESSION['PCRITICAL'].'</td> <td>'.$_SESSION['FOSOM'].'</td> <td>'.$_SESSION['FOSSLIDING'].'</td><td>'.$_SESSION['A'].' x '.$_SESSION['A'].'</td><td>'.$_SESSION['D'].'</td> </tr> <br>';

    $mail->Body.=' </table><br><br>';

    $mail->Body.='<h2>Detailing (c/c spacing)</h2>';
    $mail->AddEmbeddedImage('images/1.png', 'cccs');
    $mail->Body.='<center><img src="cid:cccs" /></center><br><br>';
    $mail->Body.= '<table style="border: 1px solid black;"> <tr>
        <th>S1 (mm)</th>
        <th>S2 (mm)</th>
        <th>S3 (mm)</th>
    ...
    $mail->Body.='<h2>Detailing in Counterfort (c/c spacing)</h2>';
    $mail->AddEmbeddedImage('images/5.jpg', 'cf');
    $mail->Body.='<center><img src="cid:cf" /></center><br><br>';
    $mail->Body.= '<table style="border: 1px solid black;"> <tr>
        <th>S1 (mm)</th>
        <th>S2 (mm)</th>
        <th>S3 (mm)</th>
        <th>Discrete Wall Length (mm)</th>
      </tr>';
    
    // $totalPrice=0;
    // while ($row = mysqli_fetch_array($resultOrder)){
    //     $totalPrice+=$row["Price"]*$row["quantity"];
         $mail->Body.='<tr> <td>'.$_SESSION['CS1'].'</td> <td>'.$_SESSION['CS2'].'</td> <td>'.$_SESSION['CS3'].'</td> <td>'.$_SESSION['DWL'].'</td>  </tr> <br>';
